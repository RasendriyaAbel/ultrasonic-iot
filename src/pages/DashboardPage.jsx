...
export function DashboardPage() {
  ...
  const estimatedDays =
    t?.consumption?.estimatedDaysRemaining == null ? '—' : `${formatNumber(t.consumption.estimatedDaysRemaining, { maximumFractionDigits: 1 })} hari`
  ...
  return (
    <div className="grid gap-5">
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
        ...
        />
        <StatCard
          title="Jarak Ultrasonik"
          value={
            sensors?.distanceCm != null
              ? `${formatNumber(sensors.distanceCm, { maximumFractionDigits: 2 })} cm`
              : '—'
          }
          subtitle="jarak_cm"
          icon={Ruler}
        ...
        <StatCard
          ...
          subtitle="Flow 2 • flow_out_lpm / flow2Lpm"
          icon={Activity}
          tone="info"
        />

        <StatCard
          title="Selisih Debit (In - Out)"
          value={formatLpm(t?.flow?.differenceLpm, 2)}
          subtitle={t?.flow?.lossPercentage == null ? '—' : `Kehilangan ${formatNumber(t.flow.lossPercentage, { maximumFractionDigits: 1 })}%`}
          icon={Sigma}
          tone={t?.flow?.differenceLpm != null && t.flow.differenceLpm > state.settings.leakDiffThresholdLpm ? 'warn' : 'neutral'}
        />
        <StatCard
          title="Status Kebocoran"
          value={t?.leakage?.status ?? '—'}
          subtitle={t?.leakage?.location ?? 'Standby saat pompa OFF'}
          icon={AlertTriangle}
        ...
        <StatCard
          ...
          subtitle="Akumulasi konsumsi (Flow Out)"
          icon={LineChart}
          tone="neutral"
        />
        <StatCard
          title="Status Pompa"
          value={t?.pump?.status ?? '—'}
          subtitle="Kontrol manual ON/OFF"
          icon={Power}
          tone={pumpTone(t?.pump?.status)}
        />
        <StatCard
          title="Estimasi Sisa Hari"
          value={estimatedDays}
          subtitle={t?.consumption?.dailyAverageLiter == null ? '—' : `Rata-rata ${formatLiter(t.consumption.dailyAverageLiter, 1)}/hari`}
          icon={Timer}
          tone="neutral"
        />
        <StatCard
          title="Error Device"
          value={sensors?.errorCount != null ? formatNumber(sensors.errorCount, 0) : '—'}
          subtitle="error_count / errorCount"
          icon={Bug}
